...
for folder in "$BASE_DIR"/*; do
  ...
  if [ -d "$folder" ]; then
    ...
    # Execute the magick montage command for the current folder
    magick montage "$folder"/* -geometry 128x128 -tile 8x8 -background transparent -filter Catrom "$BASE_DIR/$OUTPUT_FILE"
